...
const APP_DISPLAY_NAME = isDevelopment ? "Cut2Kit (Dev)" : "Cut2Kit (Alpha)";
...
function handleFatalStartupError(stage: string, error: unknown): void {
  ...
  if (!isQuitting) {
    isQuitting = true;
    dialog.showErrorBox("Cut2Kit failed to start", `Stage: ${stage}\n${message}${detail}`);
  }
  stopBackend();
  restoreStdIoCapture?.();
  app.quit();
...
async function checkForUpdatesFromMenu(): Promise<void> {
  ...
  if (updateState.status === "up-to-date") {
    void dialog.showMessageBox({
      ...
      message: `Cut2Kit ${updateState.currentVersion} is currently the newest version available.`,
      buttons: ["OK"],
    });
 ...
 * parentheses (e.g. `~/.config/T3 Code (Alpha)` on Linux for older builds). This is
 * unfriendly for shell usage and violates Linux naming conventions.
 *
 * We override it to a stable lowercase directory name (`t3code`). If the
 * legacy directory already exists we keep using it so existing users don't
